...
class HarrisKeypointDetector(KeypointDetector):
    ...
    def computeHarrisValues(self, srcImage):
        ...
        srcImage = scipy.ndimage.gaussian_filter(srcImage,mode="reflect")
        Ix = scipy.ndimage.sobel(srcImage,axis=0,mode="reflect")    #x的方向导数
        Iy = scipy.ndimage.sobel(srcImage,axis=1,mode="reflect")    #y的方向导数

        harrisImage = np.zeros(srcImage.shape[:2])
        Det = Ix**2*Iy**2-(Ix*Iy)**2
        Trace = Ix**2+Iy**2
        harrisImage = Det - 0.1*Trace
